...
# Durante o teste, os links são RELATIVOS e apontam para os .htm
# que são acessados pelos links externos definidos abaixo.
...
def extrair_titulo_do_nome_arquivo(caminho_arquivo):
    ...
    # O relatório da Acoforte pode chegar com sufixos/códigos no nome
    # (por exemplo, Acofortegdg). Todos esses nomes devem exibir apenas
    # "Acoforte" no título da página.
...
def obter_links_estoque_producao(registros, titulo_pagina=""):
    ...
    """Retorna os estoques dos clientes identificados no arquivo de produção.

    A coluna Cliente pode ser deslocada ou ficar vazia em alguns relatórios
    HTML antigos. Como a página de produção é específica do cliente, o nome
    do arquivo também é usado como fallback para garantir o botão correto.
    """
    clientes_presentes = {
    ...
    for chave, (nome, arquivo) in ARQUIVOS_ESTOQUE_CLIENTES.items():
        ...
        # Fallback robusto para o relatório da Acoforte: o próprio nome da
        # página pode conter Acoforte/Acofortegdg, mesmo quando a coluna
        # Cliente não foi reconstruída pelo HTML malformado.
...
.controls input,.controls select,.controls button{{padding:10px;border:1px solid #d8e0da;border-radius:8px;background:white;font-size:14px}}
.controls input{{flex:1;min-width:230px}}
.controls button,.controls .btn-estoque{{background:#176b28;color:white;cursor:pointer;border:none;font-weight:bold;padding:10px 20px;border-radius:8px;font-size:14px;box-sizing:border-box;font-family:inherit}}
