...
# ============================================================
# V8.6 — DASHBOARD COM CARDS INTERATIVOS (FILTRO CLICÁVEL)
# ============================================================
# CONFIGURAÇÃO
# ============================================================

PASTA_ORIGEM = Path(r"C:\FTP") # Caminho atualizado para ambiente de produção
PASTA_DESTINO = Path(r"C:\FTP 2.0") # Caminho atualizado para ambiente de produção

NOME_LOGO_EMBRACO = "logo_embo_new.png" # Adicionado: Nome do arquivo do logo
...
# Adicionado: Função para copiar o logo
def copiar_logo_embraco():
    ...
    if caminho_origem_logo.exists():
        ...
        print(f"⚠️ Logo '{NOME_LOGO_EMBRACO}' não encontrado em '{PASTA_ORIGEM}'. Certifique-se de que ele está lá.")
...
/* INÍCIO: Alteração do CSS para o logo e texto do cabeçalho */
...
/* FIM: Alteração do CSS para o logo e texto do cabeçalho */
...
<div class="wrap">
<div class="head">
    <!-- INÍCIO: Alteração do HTML para o logo e texto do cabeçalho -->
    <div class="brand">
        <img class="brand-logo"
             src="logo_embo_new.png"
             alt="Logo EMBRACO">

        <div class="brand-text">
            <strong class="brand-name">SIAPE</strong>
            <span class="brand-subtitle">
                Sistema Integrado de Acompanhamento de Produção EMBRACO
            </span>
        </div>
    </div>
    <!-- FIM: Alteração do HTML para o logo e texto do cabeçalho -->
<h1>{escape(titulo)}</h1>
...
def gerar_html_producao(titulo, data, registros):
    ...
    botoes_estoque = gerar_botoes_estoque_producao(registros, titulo)
...
/* STYLES DO DASHBOARD (V8.6 — CARDS CLICÁVEIS) */
.dash-container{{margin-bottom:14px}}
.dash-kpis{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin-bottom:14px}}
...
/* NOVO: ESTILO INTERATIVO/CLICÁVEL DOS CARDS DE STATUS */
.dash-status-item{{
    background:#f8faf8;
    padding:10px 12px;
    border-radius:8px;
...
const ALL_STATUSES = [
    {{ name: "LIBERADO", cls: "lib" }},
    {{ name: "SEPARADO", cls: "sep" }},
    {{ name: "PONTEADO", cls: "pon" }},
    {{ name: "AGUARDANDO SEPARAÇÃO", cls: "agu" }},
    {{ name: "DECAPADO", cls: "dec" }},
    {{ name: "SERRADO", cls: "ser" }},
    {{ name: "TREFILADO", cls: "tre" }},
    {{ name: "ENDIREITADO", cls: "end" }},
    {{ name: "ESPECIAL", cls: "esp" }}
];

function fmt(v) {{ return Math.round(v).toLocaleString('pt-BR') + ' KG'; }}

// FUNÇÃO AUXILIAR: Filtra via clique no Card do Dashboard
...
function updateDashboard(visibleRows) {{
    ...
    ALL_STATUSES.forEach(s => {{
        ...
        // V8.6 — Adiciona a propriedade onclick no Card
        item.onclick = () => filtrarPorStatus(s.name);
...
def processar():
    print("=" * 80)
    print("PROCESSAMENTO V8.6: CARDS DE STATUS CLICÁVEIS")
    print("=" * 80)
    ...
    # Adicionado: Copia o logo para a pasta de destino antes de processar os HTMLs
    copiar_logo_embraco()
    ...
    for arquivo in todos_arquivos:
        if arquivo.suffix.lower() == ".htm":
            continue
        # Adicionado: Ignora o arquivo do logo se ele estiver na pasta de origem
